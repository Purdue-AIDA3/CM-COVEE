...
class DataProcesser:
            ...
            for j in range(1, len(name_list)): # start from 1 to ignore the first video
                if 'Task_2' not in name_list[j]:
                    if 'secondary' not in name_list[j]:
                        psd = loadmat(
                            self.data_path + id_name + '/EEG/min_processed/Task 1 Without Secondary Task/' + name_list[
                                j] + '.mat')
                    else:
                        psd = loadmat(
                            self.data_path + id_name + '/EEG/min_processed/Task 1 With Secondary Task/' + name_list[j][
                                                                                                      0:-10] + '.mat')
        ...
        for i in range(0, len(sub_folders)):
            ...
            for j in range(1, len(name_list)):  # start from 1 to ignore the first video
                if 'Task_2' not in name_list[j]:
                    if 'secondary' not in name_list[j]:
                        psd = loadmat(
                            self.data_path + id_name + '/EEG/min_processed/Task 1 Without Secondary Task/' +
                            name_list[
                                j] + '.mat')
                    else:
                        psd = loadmat(
                            self.data_path + id_name + '/EEG/min_processed/Task 1 With Secondary Task/' +
                            name_list[j][
                            0:-10] + '.mat')
    ...
    def prepare_test_data(self, modality, batchsize, test_levels,test_meta_psd,test_meta_pupil_diam,test_meta_gaze_speed,
                               test_meta_eye_landmarks,test_meta_face_landmarks):
